...
const START_POS = [-0.2092, 0.27, -3.0771];
...
const MIN_PITCH = -Math.PI / 4 + 0.01; // -89.99°
const MAX_PITCH = Math.PI / 4 - 0.01;  // 
const MIN_YAW   =  0.35 - (3*Math.PI)/4;          // -180°
const MAX_YAW   =  0.35  + (3*Math.PI)/4;           // 180°
...
async function main() {
    ...
    window.addEventListener("wheel", e => {
        ...
        const step = 0.05;                    // feel free to tweak
    ...
    if ((ev.buttons & 1) || (ev.pointerType === 'touch' && ev.isPrimary)) {
        const dx = ROTATE_SPEED * (ev.clientX - px) / innerWidth;
        const dy = ROTATE_SPEED * (ev.clientY - py) / innerHeight;

        targetYaw   = clamp(targetYaw   + dx, MIN_YAW,   MAX_YAW);
        targetPitch = clamp(targetPitch - dy, MIN_PITCH, MAX_PITCH);

        px = ev.clientX;                 // remember for the next move
        py = ev.clientY;
    }
